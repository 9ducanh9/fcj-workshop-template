---
title: "Báo cáo đồ án cuối khóa FCAJ AWS Bootcamp"
date: 2026-05-12
weight: 1
chapter: false
---

# Báo cáo đồ án cuối khóa FCAJ AWS Bootcamp

## Cognitive Communication Coach trên AWS

Báo cáo này trình bày đồ án cuối khóa FCAJ AWS Bootcamp của tôi: một hệ thống AI serverless giúp người dùng xem lại hội thoại, nhận diện điểm yếu trong lập luận, và luyện tập phản hồi rõ ràng hơn sau một buổi trao đổi, phỏng vấn, họp, hoặc thuyết trình.

Đồ án được thiết kế theo hướng **phản tư sau hội thoại** thay vì trợ lý thời gian thực. Cách tiếp cận này thực tế hơn cho một học viên, dễ triển khai lại, an toàn hơn về quyền riêng tư, nhưng vẫn giữ tinh thần ban đầu: dùng AI để hỗ trợ con người tư duy tốt hơn, không thay thế con người.

Hãy bổ sung ảnh chụp màn hình triển khai và bằng chứng chạy thực tế trước khi nộp chính thức.

## Thông tin học viên

| Trường thông tin | Nội dung |
| --- | --- |
| Họ và tên | Lâm Chí Tài |
| Số điện thoại | 0798353782 |
| Email | lamchitai2300@gmail.com |
| Trường đại học | HUTECH - Đại học Công nghệ TP.HCM |
| Chuyên ngành | Hệ thống thông tin (Information Systems) |
| Công ty thực tập | Amazon Web Services Vietnam |
| Vị trí thực tập | Workforce Bootcamp Intern |
| Thời gian thực tập | 17/04/2026 - 10/07/2026 |

![Ảnh đại diện](/images/avatar.png)

## Nội dung báo cáo

1. [Nhật ký tuần 1-12](1-Worklog/)
2. [Đề xuất dự án](2-Proposal/)
3. [Bài viết kỹ thuật](3-BlogsTranslated/)
4. [Sự kiện đã tham gia](4-EventParticipated/)
5. [Workshop kỹ thuật / Dự án chính](5-Workshop/)
6. [Tự đánh giá](6-Self-evaluation/)
7. [Chia sẻ và phản hồi](7-Feedback/)
